...
class QdrantEngineBehaviorTest {
    ...
    @Test
    void buildQueryJsonIncludesVectorParamsAndFilter() throws Exception {
        QdrantEngine e = new QdrantEngine(new HashMap<>());
        List<Float> vec = List.of(1.0f, 2.0f, 3.0f, 4.0f);
        SearchPoints sp = SearchPoints.newBuilder()
                .setCollectionName("col")
                .setLimit(10L)
                .setParams(io.qdrant.client.grpc.Points.SearchParams.newBuilder().setHnswEf(64).build())
                .setFilter(Filter.newBuilder()
                        .addMust(io.qdrant.client.grpc.Common.Condition.newBuilder()
                                .setField(io.qdrant.client.grpc.Common.FieldCondition.newBuilder()
                                        .setKey("f")
                                        .setMatch(Match.newBuilder().setKeyword("x").build())
                                        .build())
                                .build())
                        .build())
                .build();
        String json = (String) invokePrivate(e, "buildQueryJson", new Class[]{SearchPoints.class, List.class}, sp, vec);
        assertTrue(json.contains("hnsw_ef"));
        assertTrue(json.contains("filter"));
        assertTrue(json.contains("limit"));
    }

    @Test
    void buildQueryJsonTruncatesLongVectorSummary() throws Exception {
        QdrantEngine e = new QdrantEngine(new HashMap<>());
        List<Float> vec = List.of(1.0f, 2.0f, 3.0f, 4.0f, 5.0f);
        SearchPoints sp = SearchPoints.newBuilder().setCollectionName("c").setLimit(1L).build();
        String json = (String) invokePrivate(e, "buildQueryJson", new Class[]{SearchPoints.class, List.class}, sp, vec);
        assertTrue(json.contains("more"));
    }

    @Test
    void buildQueryJsonShortVectorArrayBranch() throws Exception {
        QdrantEngine e = new QdrantEngine(new HashMap<>());
        List<Float> vec = List.of(9.0f, 8.0f);
        SearchPoints sp = SearchPoints.newBuilder().setCollectionName("c").setLimit(5L).build();
        String json = (String) invokePrivate(e, "buildQueryJson", new Class[]{SearchPoints.class, List.class}, sp, vec);
        assertTrue(json.contains("9.0"));
        assertTrue(json.contains("8.0"));
        assertFalse(json.contains("more"));
    }

    @Test
    void buildQueryJsonNoFilterNoParams() throws Exception {
        QdrantEngine e = new QdrantEngine(new HashMap<>());
        List<Float> vec = List.of(1.0f, 2.0f, 3.0f);
        SearchPoints sp = SearchPoints.newBuilder().setCollectionName("c").setLimit(7L).build();
        String json = (String) invokePrivate(e, "buildQueryJson", new Class[]{SearchPoints.class, List.class}, sp, vec);
        assertFalse(json.contains("\"filter\""));
        assertFalse(json.contains("\"params\""));
        assertTrue(json.contains("\"limit\" : 7"));
    }

    @Test
    void buildQueryJsonFilterWithBooleanAndIntegerMatchCases() throws Exception {
        QdrantEngine e = new QdrantEngine(new HashMap<>());
        List<Float> vec = List.of(1.0f, 2.0f);
        Match boolMatch = Match.newBuilder().setBoolean(true).build();
        Match intMatch = Match.newBuilder().setInteger(99L).build();
        SearchPoints sp = SearchPoints.newBuilder()
                .setCollectionName("col")
                .setLimit(2L)
                .setFilter(Filter.newBuilder()
                        .addMust(Condition.newBuilder()
                                .setField(FieldCondition.newBuilder().setKey("fb").setMatch(boolMatch).build())
                                .build())
                        .addMust(Condition.newBuilder()
                                .setField(FieldCondition.newBuilder().setKey("fi").setMatch(intMatch).build())
                                .build())
                        .build())
                .build();
        String json = (String) invokePrivate(e, "buildQueryJson", new Class[]{SearchPoints.class, List.class}, sp, vec);
    ...
    @Test
    void buildQueryJsonSkipsFieldWhenConditionHasNoField() throws Exception {
        QdrantEngine e = new QdrantEngine(new HashMap<>());
        List<Float> vec = List.of(1.0f);
        SearchPoints sp = SearchPoints.newBuilder()
                .setCollectionName("col")
                .setLimit(1L)
                .setFilter(Filter.newBuilder()
                        .addMust(Condition.newBuilder().build())
                        .build())
                .build();
        String json = (String) invokePrivate(e, "buildQueryJson", new Class[]{SearchPoints.class, List.class}, sp, vec);
        assertFalse(json.contains("\"filter\""));
    ...
    @Test
    void buildQueryJsonSkipsMatchBlockWhenFieldHasNoMatch() throws Exception {
        QdrantEngine e = new QdrantEngine(new HashMap<>());
        List<Float> vec = List.of(1.0f);
        SearchPoints sp = SearchPoints.newBuilder()
                .setCollectionName("col")
                ...
                .setFilter(Filter.newBuilder()
                        .addMust(Condition.newBuilder()
                                .setField(FieldCondition.newBuilder().setKey("only").build())
                                .build())
                        .build())
                .build();
        String json = (String) invokePrivate(e, "buildQueryJson", new Class[]{SearchPoints.class, List.class}, sp, vec);
        assertTrue(json.contains("\"filter\""));
        assertFalse(json.contains("\"match\""));
    }

    @Test
    void buildQueryJsonParamsWithQuantizationOnlyNoHnswEf() throws Exception {
        QdrantEngine e = new QdrantEngine(new HashMap<>());
        List<Float> vec = List.of(1.0f);
        SearchPoints sp = SearchPoints.newBuilder()
                .setCollectionName("col")
                .setLimit(1L)
                .setParams(io.qdrant.client.grpc.Points.SearchParams.newBuilder()
                        .setQuantization(QuantizationSearchParams.newBuilder().setOversampling(2.0).build())
                        .build())
                .build();
        String json = (String) invokePrivate(e, "buildQueryJson", new Class[]{SearchPoints.class, List.class}, sp, vec);
        assertTrue(json.contains("oversampling"));
        assertFalse(json.contains("hnsw_ef"));
    }

    @Test
    void buildQueryJsonWithPayloadDisabled() throws Exception {
        QdrantEngine e = new QdrantEngine(new HashMap<>());
        List<Float> vec = List.of(1.0f);
        SearchPoints sp = SearchPoints.newBuilder()
                .setCollectionName("col")
                ...
                .build();
        String json = (String) invokePrivate(e, "buildQueryJson", new Class[]{SearchPoints.class, List.class}, sp, vec);
        assertTrue(json.contains("\"with_payload\" : false"));
    }

    @Test
    void buildQueryJsonMatchSwitchDefaultUsesTextMatch() throws Exception {
        QdrantEngine e = new QdrantEngine(new HashMap<>());
        List<Float> vec = List.of(1.0f, 2.0f);
        Match textMatch = Match.newBuilder().setText("hello").build();
        SearchPoints sp = SearchPoints.newBuilder()
                .setCollectionName("col")
                .setLimit(2L)
                .setFilter(Filter.newBuilder()
    ...
    @Test
    void buildQueryJsonIncludesQuantizationParamsAndWithPayload() throws Exception {
        QdrantEngine e = new QdrantEngine(new HashMap<>());
        List<Float> vec = List.of(1.0f, 2.0f);
        SearchPoints sp = SearchPoints.newBuilder()
                .setCollectionName("col")
                ...
                .setParams(io.qdrant.client.grpc.Points.SearchParams.newBuilder()
                        .setHnswEf(8)
                        .setQuantization(QuantizationSearchParams.newBuilder().setOversampling(1.5).build())
                        .build())
                .setWithPayload(WithPayloadSelector.newBuilder().setEnable(true).build())
                .build();
        String json = (String) invokePrivate(e, "buildQueryJson", new Class[]{SearchPoints.class, List.class}, sp, vec);
        assertTrue(json.contains("oversampling"));
        assertTrue(json.contains("with_payload"));
        assertTrue(json.contains("\"with_payload\" : true"));
    }

    @Test
    void buildQueryJsonReturnsErrorMessageOnNullVector() throws Exception {
        QdrantEngine e = new QdrantEngine(new HashMap<>());
        SearchPoints sp = SearchPoints.newBuilder().setCollectionName("c").setLimit(1L).build();
        String json = (String) invokePrivate(e, "buildQueryJson", new Class[]{SearchPoints.class, List.class}, sp, null);
        assertTrue(json.startsWith("Error building query JSON:"));
    }
}
